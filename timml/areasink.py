...
class AreaSinkInhom(Element):
    def __init__(self, model, xc, yc, recharge=0.001, layer=0, aqin=None, name='AreaSinkInhom', label=None):
        Element.__init__(self, model, nparam=1, nunknowns=0, layers=layer, \
                         name=name, label=label)
        self.xc = xc
        self.yc = yc
    ...
    def __repr__(self):
        return self.name + ' with recharge ' + str(self.recharge)
    ...
    def initialize(self):
        ...
        if self.aq.ilap:
            ...
            print('AreaSinkInhom cannot be added to semi-confined system')
            
    def potinf(self, x, y, aq=None):
        rv = np.zeros((self.nparam, aq.naq))
        if self.aq.isinside(x, y):
            rv[0, 0] = -0.25 * ((x - self.xc) ** 2 + (y - self.yc) ** 2)
            rv[0, 1:] = self.C
        return rv
